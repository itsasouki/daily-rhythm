...
function renderAccount(){const signed=!!cloudUser;document.getElementById('accountTitle').innerHTML=signed?'Your rhythm<br>is connected.':'Carry your rhythm<br>between devices.';document.getElementById('accountCopy').textContent=signed?cloudUser.email:'Sign in with your email and password. Your existing history will merge into your account.';document.getElementById('authForm').hidden=signed;document.getElementById('authMode').hidden=signed;document.getElementById('signOut').hidden=!signed;document.getElementById('accountOpen').textContent=signed?'synced':'sync'}
...
function setAuthMode(create){creatingAccount=create;document.getElementById('authSubmit').textContent=create?'create account':'sign in';document.getElementById('authMode').textContent=create?'I already have an account':'create an account';document.getElementById('authPassword').autocomplete=create?'new-password':'current-password';showStatus('')}
document.getElementById('authMode').onclick=()=>setAuthMode(!creatingAccount);
document.getElementById('authForm').onsubmit=async e=>{e.preventDefault();showStatus(creatingAccount?'Creating account…':'Signing in…');const email=document.getElementById('authEmail').value.trim(),password=document.getElementById('authPassword').value;const {data,error}=creatingAccount?await cloud.auth.signUp({email,password}):await cloud.auth.signInWithPassword({email,password});if(error){showStatus(error.message);return}if(creatingAccount&&!data.session){showStatus('Account created. Check your email once to confirm it, then return and sign in.')}else showStatus('Connected. Syncing your rhythm…')};
