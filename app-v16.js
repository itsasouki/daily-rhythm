...
function renderAccount(){const signed=!!cloudUser;document.getElementById('accountTitle').innerHTML=signed?'Your rhythm<br>is connected.':'Carry your rhythm<br>between devices.';document.getElementById('accountCopy').textContent=signed?cloudUser.email:'Sign in with your email and password. Your existing history will merge into your account.';document.getElementById('authForm').hidden=signed;document.getElementById('createAccount').hidden=signed;document.getElementById('signOut').hidden=!signed;document.getElementById('accountOpen').textContent=signed?'synced':'sync'}
...
async function authenticate(create){const email=document.getElementById('authEmail').value.trim(),password=document.getElementById('authPassword').value;if(!email||password.length<6){showStatus('Enter your email and a password of at least 6 characters.');return}showStatus(create?'Creating account…':'Signing in…');const {data,error}=create?await cloud.auth.signUp({email,password}):await cloud.auth.signInWithPassword({email,password});if(error){showStatus(error.message);return}if(create&&!data.session)showStatus('Account created. Check your email once to confirm it, then return and sign in.');else showStatus('Connected. Syncing your rhythm…')}
document.getElementById('authForm').onsubmit=e=>{e.preventDefault();authenticate(false)};
